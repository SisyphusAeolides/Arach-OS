...
idris_source="$root/formal/idris2/ImageComposition.idr"
agda_source="$root/formal/agda/ImageComposition.agda"
...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/arach-os-formal.XXXXXXXX")"
trap 'rm -rf -- "$scratch"' EXIT
cp "$idris_source" "$agda_source" "$scratch/"
(
    cd "$scratch"
    "$idris" --check ImageComposition.idr
    XDG_DATA_HOME="$scratch/data" XDG_CONFIG_HOME="$scratch/config" \
        "$agda" --no-libraries --safe --without-K ImageComposition.agda
)
